...
const data = readFile('./Fetch.json', 'utf8')
    .then((data) => {
        ...
        async function execute() {
               do {
                   const location = await prompt("Please enter a location: ");
                   console.log();
                   ...
                   const question = await prompt ("If you want to check another location please enter Y or what else for exit: ");
                   console.log();
                   if (question === "Y"){
                       isValid;
                   } else{
